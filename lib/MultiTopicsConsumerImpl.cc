...
MultiTopicsConsumerImpl::MultiTopicsConsumerImpl(ClientImpl& client, const std::vector<std::string>& topics,
...
void MultiTopicsConsumerImpl::beforeConnectionChange(ClientConnection& cnx) {
    consumers_.forEachValue(
        [&cnx](const ConsumerImplPtr& consumer) { consumer->beforeConnectionChange(cnx); });
}
